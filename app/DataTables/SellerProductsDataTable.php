...
class SellerProductsDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', 'sellerproducts.action')->addColumn('action', function ($query) {
            ...
            ->rawColumns(['image', 'product_type', 'status', 'approve', 'action'])
            ->setRowId('id');
    }
    ...
    {
        return $model->where('vendor_id', '!=', Auth::user()->vendorprofile->id)
            ->where('is_approved', 1)
            ->newQuery();
    }

    /**
     * Optional method if you want to use the html bui-lder.
     */
